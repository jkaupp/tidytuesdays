...
tree <- ggplot(plot_data) +
  ...
  #geom_mark_circle(data = top, aes(x = x, y = y, label = species, description = glue('{n} Trees Planted')), expand = unit(0, "mm"), label.family = c("Oswald", "Lora"), con.size = c(0, rep(0.5, 4))) +
  geom_text(data = top, aes(x = x, y = y, label = glue("{str_wrap(species, 10)}\n{n} planted")), family = "Oswald", color = c("white", "grey20", "white", "grey20")) +
